...
import com.wardtn.uvccamera.utils.BuildCheck;
import com.wardtn.uvccamera.utils.HandlerThreadHandler;
import com.wardtn.uvccamera.utils.XLogWrapper;
...
public final class USBMonitor {
    ...
    /**
     * 用于注册一个 BroadcastReceiver 以监控 USB 设备的 连接 和 断开事件
     *
    ...
    public synchronized void register() throws IllegalStateException {
        ...
        if (mPermissionIntent == null) {
            ...
            if (context != null) {
                ...
                filter.addAction(ACTION_USB_DEVICE_ATTACHED); // USB 设备附加
...
//		if (DEBUG) XLogWrapper.v(TAG, "requestPermission:device=" + device);
        ...
        if (isRegistered()) {
            if (device != null) {
                if (DEBUG)
                    XLogWrapper.i(TAG, "request permission, has permission: " + mUsbManager.hasPermission(device));
                ...
                synchronized (USBMonitor.this) {
                    ...
                    if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
                        if (device != null) {
                            ...
                            if (DEBUG) XLogWrapper.w(TAG, "get permission success in mUsbReceiver");
                            processConnect(device);
                        }
                    } else {
                        // failed to get permission
                        if (DEBUG) XLogWrapper.w(TAG, "get permission failed in mUsbReceiver");
                        processCancel(device);
                    }
                }
    ...
    // 当前连接和检测到的 USB 设备的数量
    // 使用 volatile 确保线程安全, 避免多线程环境下的同步问题
    private volatile int mDeviceCounts = 0;

    /**
     * mDeviceCheckRunnable 定期检查 USB 设备的连接状态，
     * 并在设备列表发生变化时调用监听器的 onAttach 方法。
     * 这种定期检查机制可以确保应用程序能够及时响应设备的连接和断开变化。
     */
    ...
    private void processConnect(final UsbDevice device) {
        if (destroyed) return;
        // 调用 updatePermission 方法来更新设备的权限
        updatePermission(device, true);
        // 使用 mAsyncHandler 异步执行后续代码块 这确保了处理 USB 设备的操作不会阻塞主线程。
        mAsyncHandler.post(() -> {
            if (DEBUG) XLogWrapper.v(TAG, "processConnect:device=" + device.getDeviceName());
            // 获取或创建 UsbControlBlock 对象
            //从 mCtrlBlocks 中获取与设备关联的 UsbControlBlock 对象。
            //如果 ctrlBlock 为 null，表示这是一个新的设备，创建一个新的 UsbControlBlock 实例，并将其存储到 mCtrlBlocks 中，同时标记为新创建（createNew = true）。
            //如果 ctrlBlock 已经存在，则标记为不是新创建的（createNew = false）
            UsbControlBlock ctrlBlock;
...
//		if (DEBUG) XLogWrapper.v(TAG, "getDeviceKeyName:" + sb.toString());
    ...
    public static class UsbDeviceInfo {
        public String usb_version; // USB 版本号
    ...
    public static final class UsbControlBlock implements Cloneable {
        ...
        protected UsbDeviceConnection mConnection; //USB连接信息
        protected final UsbDeviceInfo mInfo; // USB 设备信息
        private final int mBusNum; //总线号
        private final int mDevNum; //设备号
        ...
        private UsbControlBlock(final USBMonitor monitor, final UsbDevice device) {
            ...
            // device.getDeviceName() 返回 USB 设备的名称，通常是一个表示设备路径的字符串，例如 /dev/bus/usb/001/002。
