...
import com.wardtn.uvccamera.EffectListDialog.Companion.KEY_ANIMATION
import com.wardtn.uvccamera.EffectListDialog.Companion.KEY_FILTER
import com.wardtn.uvccamera.callback.ICameraStateCallBack
import com.wardtn.uvccamera.callback.ICaptureCallBack
import com.wardtn.uvccamera.callback.IPlayCallBack
import com.wardtn.uvccamera.camera.CameraUVC
import com.wardtn.uvccamera.databinding.DialogMoreBinding
import com.wardtn.uvccamera.databinding.FragmentDemoBinding
import com.wardtn.uvccamera.render.effect.EffectBlackWhite
import com.wardtn.uvccamera.render.effect.EffectSoul
import com.wardtn.uvccamera.render.effect.EffectZoom
...
import com.wardtn.uvccamera.utils.bus.BusKey
import com.wardtn.uvccamera.utils.bus.EventBus
import com.wardtn.uvccamera.utils.imageloader.ILoader
import com.wardtn.uvccamera.utils.imageloader.ImageLoaders
import com.wardtn.uvccamera.widget.*
...
class DemoFragment  : CameraFragment(), View.OnClickListener, CaptureMediaView.OnViewClickListener{
    ...
    private fun handleCameraError(msg: String?) {
        ...
        ToastUtils.showLong("camera opened error: $msg")
    }
    ...
    private fun handleCameraOpened() {
        ...
        Log.e("CHEN", "max = ${mViewBinding.brightnessSb.max}, progress = ${mViewBinding.brightnessSb.progress}")
        ...
        ToastUtils.showLong("camera opened success")
    }
    ...
    override fun onViewClick(mode: CaptureMediaView.CaptureMode?) {
        if (! isCameraOpened()) {
            ToastUtils.showLong("camera not worked!")
            return
        }
        when (mode) {
    ...
    private fun captureAudio() {
        ...
        captureAudioStart(object : ICaptureCallBack {
            ...
            override fun onError(error: String?) {
                ToastUtils.showLong(error ?: "未知异常")
            ...
            override fun onComplete(path: String?) {
                ...
                mViewBinding.recTimerLayout.visibility = View.GONE
                stopMediaTimer()
                ToastUtils.showLong(path ?: "error")
            }
...
//        }
    ...
    private fun showUsbDevicesDialog(usbDeviceList: MutableList<UsbDevice>?, curDevice: UsbDevice?) {
        if (usbDeviceList.isNullOrEmpty()) {
            ToastUtils.showLong("Get usb device failed")
            return
        }
            ...
            setData(mEffectDataList, object : EffectListDialog.OnEffectClickListener {
                override fun onEffectClick(effect: CameraEffect) {
                    mEffectDataList.find {it.id == effect.id}.also {
                        if (it == null) {
                            ToastUtils.showLong("set effect failed!")
                            return@also
                        }
    ...
    private fun showResolutionDialog() {
        ...
        getAllPreviewSizes().let { previewSizes ->
            if (previewSizes.isNullOrEmpty()) {
                ToastUtils.showLong("Get camera preview size failed")
                return
            }
            val list = arrayListOf<String>()
            var selectedIndex: Int = -1
    ...
    private fun goToGalley() {
        try {
            ...
            ToastUtils.showLong("open error: ${e.localizedMessage}")
        }
    }
    ...
    private fun showRecentMedia(isImage: Boolean? = null) {
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                Log.e(TAG, "showRecentMedia failed", e)
            }
        }
    }
