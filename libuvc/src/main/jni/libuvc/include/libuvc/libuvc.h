 ...
 *
 * 「帧」是流格式的一种配置，用于指定图像尺寸，并对应于可能的多个可用帧率中的某一个。
 *
 */
typedef struct uvc_frame_desc {

    // 说明: 指向该帧所属的格式描述符（uvc_format_desc），表示该帧是某个特定格式的一部分。
    //用途: 用于将帧与父格式关联起来
    struct uvc_format_desc *parent;

    // 说明: 双向链表指针，分别指向当前帧的前一个帧和后一个帧。
    // 用途: 用于组织同一格式下的多个帧描述符
    struct uvc_frame_desc *prev, *next;


    //说明: 表示帧的类型，例如 JPEG 帧或未压缩帧。
    //用途: 确定帧的具体子类型，帮助识别帧的编码方式。
    ...
    // 对应流在最低帧率时的比特率（单位：bps）
    /** Bitrate of corresponding stream at minimal frame rate */
    uint32_t dwMinBitRate;

    // 对应流在最高帧率时的比特率（单位：bps）
    /** Bitrate of corresponding stream at maximal frame rate */
    uint32_t dwMaxBitRate;

    // 帧的最大字节数 确定缓冲区大小以容纳最大帧数据
...
typedef struct uvc_format_desc {
    ...
    // 双向链表指针，分别指向当前格式的前一个格式和后一个格式
    struct uvc_format_desc *prev, *next;

    // 表示流的类型，例如 JPEG 流或未压缩流。
    /** Type of image stream, such as JPEG or uncompressed. */
    enum uvc_vs_desc_subtype bDescriptorSubtype;

    // 当前格式在 VS 接口格式列表中的标识符
    ...
    //说明: 格式规格化信息，使用 GUID 或 FOURCC 格式表示。
    //guidFormat[16]: 使用 16 字节的 GUID 表示格式。
    //fourccFormat[4]: 使用 4 字节的 FOURCC 表示格式。
    //用途: 提供格式的唯一标识符。
