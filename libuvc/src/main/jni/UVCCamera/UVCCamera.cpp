...
int UVCCamera::connect(int vid, int pid, int fd, int busnum, int devaddr, const char *usbfs) {
	...
	// 检查当前没有打开的设备句柄且文件描述符 (fd) 有效
	if (!mDeviceHandle && fd) {
		// 释放旧的 USB 文件系统字符串并复制新的
		if (mUsbFs)
			free(mUsbFs);
		mUsbFs = strdup(usbfs);

		// 如果 UVC 上下文未初始化，初始化上下文
...
//			libusb_set_debug(mContext->usb_ctx, LIBUSB_LOG_LEVEL_DEBUG);
		...
		// 清除摄像头功能标志
		clearCameraParams();
		// 复制文件描述符，避免多次关闭时导致的错误
		fd = dup(fd);

		//  根据 vid 和 pid 通过文件描述符、总线号和设备地址查找设备
//		result = uvc_find_device2(mContext, &mDevice, vid, pid, NULL, fd);
		...
		if (LIKELY(!result)) { // 如果找到了设备
			// 尝试打开设备
			result = uvc_open(mDevice, &mDeviceHandle);
			if (LIKELY(!result)) {
				// 如果打开成功
#if LOCAL_DEBUG
				uvc_print_diag(mDeviceHandle, stderr); // 打印设备诊断信息（仅在调试模式下）
#endif
				mFd = fd; // 保存文件描述符
				// 初始化状态和按钮回调
