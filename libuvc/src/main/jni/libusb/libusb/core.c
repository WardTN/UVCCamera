...
/** \ingroup dev
 * 打开一个USB 设备,并获得相应的设备句柄。这个句柄允许调用者执行设备上的 I/O 操作。
 ...
 *
 * \param dev dev: 要打开的 USB 设备。
 * \param handle 输出参数，用于返回设备句柄的指针。如果函数成功，将会在此位置返回设备句柄。
...
int API_EXPORTED libusb_open(libusb_device *dev, libusb_device_handle **handle) {
	...
	// 为设备句柄分配内存 priv_size 是与平台相关的私有数据大小
	_handle = malloc(sizeof(*_handle) + priv_size);
	if (UNLIKELY(!_handle))
		return LIBUSB_ERROR_NO_MEM;

	// 初始化互斥锁，用于保护设备句柄的并发访问。如果初始化失败，则释放分配的内存并返回错误
	...
	// 调用 libusb_ref_device 为设备增加引用计数，确保设备在使用过程中不会被销毁。
